...
```{r, echo=FALSE}
# Expand R memory in 32-bit Windows: memory.limit(4095)
# I know, I need a machine/OS upgrade

rm(list=ls())
load("data/student2012.rda")
st <- student2012
rm(student2012)
latin.america <- c("Colombia", "Brazil", "Chile", "Argentina", "Peru", 
...
tabla.entropy <- ddply(st, .(CNT, ST04Q01), summarise, 
...
color.label.entropy <- ifelse(levels(tabla.entropy$CNT) %in% latin.america,
                              "red", "black")
...
p<- ggplot(tabla.entropy.melt, aes(x=CNT, y=value, fill=ST04Q01)) + 
      ...
      theme(axis.text.y = element_text(colour=color.label.entropy, hjust=1)) + 
...
```{r, echo=FALSE, fig.height=9, fig.width=8, fig.align='center'}
tabla.sd <- ddply(st, .(CNT, ST04Q01), summarise, sd.math=sd(PV1MATH), 
                  sd.read=sd(PV1READ))

tabla.sd$CNT <- reorder(tabla.sd$CNT, tabla.sd$sd.math)

color.label.sd <- ifelse(levels(tabla.sd$CNT) %in% latin.america, "red",
                         "black")

tabla.sd <- melt(tabla.sd, id.vars=c("CNT", "ST04Q01"), 
                 measure.vars = c("sd.math", "sd.read"))

ggplot(tabla.sd, aes(x=CNT, y=value, fill=ST04Q01)) + 
  geom_bar(stat="identity", position="dodge") + 
  facet_grid(. ~ variable) +
  coord_flip() +
  scale_fill_discrete(name="Sex") +
  theme(axis.text.y = element_text(colour=color.label.sd, hjust=1)) +
  xlab("Country") +
...
```{r, echo=FALSE}
paises <- c("Spain", "Canada",  "Israel", "Japan", "Germany", "Finland",
            "Estonia", "Ireland", "Qatar", "France", latin.america)

paises.comp <- c(latin.america, "Japan", "Finland", "Canada", "Spain",
                 "Germany", "Estonia", "France", "Portugal", "Greece")
...
```{r, echo=FALSE, fig.height=11, fig.width=9, fig.align='center'}
